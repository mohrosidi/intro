...
```r
summary(iris) # membuat ringkasan data
```
...
```r
class(iris$Species) # cek jenis data kolom 'Species' pada objek iris
```
...
```r
length(iris$Petal.Width) # panjang data kolom 'Petal.Width' pada objek iris
```
...
```r
levels(iris$Species) # cek level faktor dari kolom 'Species' pada objek iris
```
...
```r
length(levels(iris$Species)) # cek panjang atau jumlah level faktor yang tersedia pada kolom 'Species' pada objek iris
```
...
```r
unique(iris$Species) # cek elemen atau karakter unik pada kolom 'Species'
```
...
```r
length(unique(iris$Species)) # jumlah karakter unik atau faktor pada kolom 'Species'
```
...
```r
mean(iris$Sepal.Length) # menghitung nilai mean kolom 'Sepal.Length'
```
...
```r
sd(iris$Sepal.Length) # menghitung nilai simpangan baku kolom 'Sepal.Length'
```
...
```r
median(iris$Sepal.Width) # menghitung nilai median kolom 'Sepal.Width'
```
...
```r
sum(iris$Petal.Length) # menghitung jumlah atau total nilai data pada kolom 'Petal.Length`
```
...
```r
write.csv(iris, file = "iris.csv", row.names = FALSE) # menyimpan hasil kedalam format file .csv
```
